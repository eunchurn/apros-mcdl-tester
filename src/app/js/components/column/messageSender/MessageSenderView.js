...
class MessageSender extends Component {
  ...
  changeType(e) {
    ...
    switch (type) {
      ...
      case "Array":
        ...
        if (
          parsedValue &&
          Object.prototype.toString.apply(parsedValue).slice(8, -1) === type
        )
          isValid = true;
        else isValid = false;
        break;
      ...
      case "String":
        try {
          // stringify current value in case it's an object or it'll look like "[object Object]", if JSON.parse works it's already a string, so we don't need to change it
          JSON.parse(value);
          value = value + ""; // convert boolean to string or codemirror throws an error
        } catch (e) {
          // if it throws we know it's not a JSON string already and we have to stringify it
      ...
      case "JSON":
        try {
          // if JSON.parse works it's valid JSON
          JSON.parse(value);
        } catch (e) {
          isValid = false;
        }
   ...
   */
  handleMessageSend() {
    if (!this.state.tab.connected) return;

    if (this.state.eventName)
      this.props.sendMessage(
        this.props.activeTab,
        this.state.eventName,
        this.state.messageCollection
      );
  }
  ...
  removeMessageArgument() {
    ...
    const newMessageCollection = [].concat(
      messageCollection.slice(0, editing),
      messageCollection.slice(editing + 1)
    );
  ...
  render() {
    ...
    const messageInEditor = state.messageInEditor;
    const messageInEditorObject =
      state.messageCollection[state.messageInEditor];
    ...
    if (
      ...
      !state.eventName ||
      (~messageInEditor &&
        !state.messageCollection.map(m => m.isValid).reduce((a, b) => a && b))
    )
      sendIsEnabled = false;

    return (
      <div className="column-block">
        <h3 className="column-title">변수 설정</h3>
        <div className="column-block-inputwrapper">
                ...
                {state.messageCollection.map((m, i) => (
                  <button
                    ...
                    className={`message-arguments-buttons-button ${
                      messageInEditor === i ? "active" : ""
                    }`}
                    onClick={() => this.setState({ messageInEditor: i })}
                  >
                    {i + 1}
                  </button>
                ))}
          ...
          {~messageInEditor ? (
            <Fragment>
              <div className="column-string">
                <span>
                  <select
                    value={messageInEditorObject.type}
              ...
              <button className="column-button" onClick={this.handleClearClick}>
                비우기
              </button>

              <Editor
                message={messageInEditorObject}
